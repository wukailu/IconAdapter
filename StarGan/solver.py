...
class Solver(object):
    ...
    def build_model(self):
        ...
        if self.dataset in ['CelebA', 'RaFD']:
            self.G = Generator(self.g_conv_dim, self.c_dim, self.g_repeat_num, self.channel)
            self.D = Discriminator(self.image_size, self.d_conv_dim, self.c_dim, self.d_repeat_num, self.channel)
        elif self.dataset in ['Both']:
            self.G = Generator(self.g_conv_dim, self.c_dim + self.c2_dim + 2, self.g_repeat_num,
                               self.channel)  # 2 for mask vector.
    ...
    def test(self):
        if self.scale_weight_file is not None:
            self.test_x4()
            return
    ...
    def test_x4(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                x_fake_list_x4 = [
                    scaleup(model, convert_image(self.denorm(x_real), nrow=1, padding=0, op=1)).astype(np.uint8)]
                for c_trg in c_trg_list:
                    x_fake_list.append(convert_image(self.denorm(self.G(x_real, c_trg)), nrow=1, padding=0))
                    x_fake_list_x4.append(scaleup(model,
                                                  convert_image(self.denorm(self.G(x_real, c_trg)), nrow=1, padding=0,
                                                                op=1)).astype(np.uint8))
